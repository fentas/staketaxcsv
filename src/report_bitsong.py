...
def txone(wallet_address, txid):
    if localconfig.legacy:
        elem = bitsong.api_cosmostation.get_tx(txid)
    else:
        elem = bitsong.api_lcd.get_tx(txid)

    print("Transaction data:")
    pprint.pprint(elem)

    exporter = Exporter(wallet_address)
    bitsong.processor.process_tx(wallet_address, elem, exporter)
    return exporter
...
def txhistory(wallet_address, job=None, options=None):
    ...
    if localconfig.cache:
        if CacheChain("bitsong", wallet_address) is None:
    ...
    progress.report_message(f"Processing {len(elems)} bitsong transactions... ")
...
def _fetch_txs(wallet_address, progress):
    if localconfig.debug:
        debug_file = f"_reports/testbitsong.{wallet_address}.json"
    ...
    while offset is not None:
        ...
        elems, offset, _ = bitsong.api_lcd.get_txs(wallet_address, offset)
