...
from secret.constants import CUR_SECRET, CURRENCIES, MILLION
from secret.make_tx import make_secret_reward_tx, make_transfer_receive_tx

# from common.TxInfo import TxInfo
from secret.TxInfo import TxInfo
...
def process_txs(wallet_address, elems, exporter):
    ...
    ErrorCounter.log(TICKER_SECRET, wallet_address)
...
def process_tx(wallet_address, elem, exporter):
    ...
    fee = _get_fee(elem)
    url = f"https://www.mintscan.io/secret/txs/{txid}"
...
def handle_del_reward(exporter, txinfo, elem, msg_index, msg_type):
    ...
    if reward:
        row = make_secret_reward_tx(txinfo, reward)
        exporter.ingest_row(row)
        return
    ...
    if transfers_in:
        for amount, currency, _, _ in transfers_in:
            row = make_secret_reward_tx(txinfo, amount)
            exporter.ingest_row(row)
        return
...
def _secret(uscrt):
    """
    Example: '5340003uscrt' -> 5.340003
    """
    amount, currency = _amount(uscrt)
    assert currency == CUR_SECRET
    return amount


def _amount(amount_string):
    """
    Example: '5340003uscrt' -> 5.340003, SCRT
    """
    if amount_string == "":
        return 0, None
