...
from bitsong.config import localconfig
from bitsong.constants import CUR_BITSONG, CURRENCIES, MILLION
from bitsong.make_tx import make_bitsong_reward_tx, make_transfer_receive_tx

# from common.TxInfo import TxInfo
from bitsong.TxInfo import TxInfo
...
from settings_csv import TICKER_BITSONG


def process_txs(wallet_address, elems, exporter):
    for elem in elems:
        process_tx(wallet_address, elem, exporter)
    ErrorCounter.log(TICKER_BITSONG, wallet_address)
...
def process_tx(wallet_address, elem, exporter):
    ...
    fee = _get_fee(elem)
    url = f"https://www.mintscan.io/bitsong/txs/{txid}"
...
def handle_del_reward(exporter, txinfo, elem, msg_index, msg_type):
    ...
    if reward:
        row = make_bitsong_reward_tx(txinfo, reward)
        exporter.ingest_row(row)
        return
    ...
    if transfers_in:
        for amount, currency, _, _ in transfers_in:
            row = make_bitsong_reward_tx(txinfo, amount)
            exporter.ingest_row(row)
        return
...
def _bitsong(ubitsong):
    """
    Example: '5340003ubtsg' -> 5.340003
    """
    amount, currency = _amount(ubitsong)
    assert currency == CUR_BITSONG
    return amount


def _amount(amount_string):
    """
    Example: '5340003ubtsg' -> 5.340003, BTSG
    """
    if amount_string == "":
        return 0, None
