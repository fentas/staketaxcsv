...
def get_tx(txid):
    uri_path = f"/bitsong/tx/v1beta1/txs/{txid}"
    data = _query(uri_path, {})
    return data.get("tx_response", None)


def _account_exists(wallet_address):
    uri_path = f"/bitsong/auth/v1beta1/accounts/{wallet_address}"
    data = _query(uri_path, {})
    return data
...
def _get_txs(wallet_address, is_sender, offset, sleep_seconds):
    uri_path = "/bitsong/tx/v1beta1/txs"
    query_params = {
